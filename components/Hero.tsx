...
import { ArrowDownRight, Globe, Layers, Hexagon, Code2, PenTool, Sparkles, Mouse } from 'lucide-react';
...
const Hero: React.FC<HeroProps> = ({ scrollToServices, scrollToContact }) => {
  ...
  return (
    <section className="relative min-h-screen flex flex-col justify-center pt-28 pb-12 overflow-hidden perspective-1000">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-gradient-conic from-brand-900/20 via-purple-900/10 to-brand-900/20 rounded-full blur-[100px] animate-spin-slow opacity-60 pointer-events-none"></div>

      {/* Floating Decorative Glass Elements */}
      <div className="absolute top-[15%] right-[15%] w-40 h-40 ultra-glass rounded-[3rem] flex items-center justify-center animate-float z-0 opacity-60 hover:opacity-100 transition-opacity duration-700">
        <div className="absolute inset-0 bg-gradient-to-tr from-white/5 to-transparent rounded-[3rem]"></div>
      </div>
      <div className="absolute bottom-[20%] left-[10%] w-24 h-24 ultra-glass rounded-[2rem] flex items-center justify-center animate-float-delayed z-0 opacity-50 hover:opacity-80 transition-opacity duration-700">
      ...
      <div className="container mx-auto px-6 z-10 mt-12">
        <div className="max-w-7xl mx-auto">
          {/* Main Typography */}
          <h1 className="text-[11vw] md:text-[8.5rem] lg:text-[10rem] font-bold leading-[0.85] tracking-tighter text-white mb-10 drop-shadow-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-12 items-end">
            <div className="md:col-span-7 animate-fade-in-up [animation-delay:400ms] opacity-0">
               <div className="pl-2 border-l-2 border-brand-500/50">
                 <p className="text-xl md:text-3xl text-zinc-300 font-light leading-relaxed max-w-2xl">
            ...
            <div className="md:col-span-5 flex flex-col gap-8 animate-fade-in-up [animation-delay:600ms] opacity-0 items-start md:items-end">
              <div className="flex flex-wrap gap-2 md:justify-end">
                 {tags.map((tag) => (
                    <div key={tag.label} className="group flex items-center gap-2 px-5 py-2.5 rounded-full ultra-glass bg-white/0 hover:bg-white/5 transition-all duration-500 cursor-default hover:scale-105">
                      {tag.icon}
              ...
              <div className="flex flex-wrap gap-4 mt-4">
                <button 
                  ...
                  className="group relative px-10 py-5 bg-white text-black font-bold text-lg rounded-full overflow-hidden transition-all duration-500 hover:scale-[1.03] hover:shadow-[0_0_50px_rgba(255,255,255,0.4)]"
                >
                ...
                <button 
                  ...
                  className="px-10 py-5 text-white rounded-full ultra-glass hover:bg-white/10 transition-all duration-500 hover:scale-[1.03] flex items-center gap-3 group"
                >
      ...
      <div className="absolute bottom-10 left-0 right-0 flex justify-center pointer-events-none animate-fade-in [animation-delay:1200ms] opacity-0">
