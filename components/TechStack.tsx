...
const TechStack: React.FC = () => {
  return (
    <section className="py-12 relative overflow-hidden reveal">
      ...
      <div className="absolute inset-0 mac-glass border-y border-white/10 z-0"></div>
      ...
      <div className="relative flex w-full overflow-hidden group z-10">
         ...
         <div className="flex min-w-full hover-pause">
            ...
            <div className="flex gap-4 md:gap-8 px-2 md:px-4 shrink-0 animate-marquee">
               {tools.map((tool, idx) => (
               <span key={`1-${idx}`} className="mac-glass px-5 py-2.5 rounded-full text-zinc-400 font-mono text-xs md:text-sm border border-white/5 hover:bg-white/10 hover:text-white transition-all cursor-default whitespace-nowrap hover:border-white/20 hover:scale-105">
                  {tool}
               </span>
               ))}
            </div>
            ...
            <div className="flex gap-4 md:gap-8 px-2 md:px-4 shrink-0 animate-marquee">
               {tools.map((tool, idx) => (
               <span key={`2-${idx}`} className="mac-glass px-5 py-2.5 rounded-full text-zinc-400 font-mono text-xs md:text-sm border border-white/5 hover:bg-white/10 hover:text-white transition-all cursor-default whitespace-nowrap hover:border-white/20 hover:scale-105">
                  {tool}
               </span>
               ))}
            </div>
